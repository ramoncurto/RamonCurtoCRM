...
@app.get("/athletes/{athlete_id}/timeline", response_class=HTMLResponse)
async def athlete_timeline(request: Request, athlete_id: int) -> HTMLResponse:
    """View athlete timeline page."""
    try:
        # Get athlete info
        ...
        logger.error(f"Error viewing athlete timeline: {e}")
...
async def process_incoming_message(phone: str, message: str, source: str, external_id: str = None) -> dict:
    ...
    try:
        ...
        event = MessageEvent(
            ...
            source_message_id=external_id or f"{source}_{datetime.now().timestamp()}",
...
async def test_webhook(request: Request) -> JSONResponse:
    ...
    try:
        ...
        result = await process_incoming_message(
            ...
            message=message,
            source=source,
            external_id="test_" + datetime.now().strftime("%Y%m%d%H%M%S")
        )
...
async def get_history_data(
    ...
    """
    Get history data in JSON format for the frontend.
    
    Parameters
    ----------
    platform : str
        Filter by platform (all, whatsapp, telegram, email, manual)
    search : str
    ...
    -------
    JSONResponse
        History data with records and metadata
    """
    try:
        with conn:
            # Build the query with filters
            query = """
                SELECT r.id, r.timestamp, r.transcription, r.final_response, r.source,
                       r.athlete_id, r.category, r.priority, r.notes,
                       a.name as athlete_name, a.sport, a.level
                FROM records r
            ...
            # Add search filter
            if search:
                query += " AND (r.transcription LIKE ? OR r.final_response LIKE ? OR a.name LIKE ?)"
            ...
            for row in rows:
                try:
                    ...
                    # Parse timestamp
                    timestamp = row[1] if row_length > 1 and row[1] else ""
                    date_obj = None
                    if timestamp:
                        try:
                    ...
                    source = row[4] if row_length > 4 and row[4] else "manual"
                    platform_info = {
                        "whatsapp": {"icon": "comment", "name": "WhatsApp"},
                        "telegram": {"icon": "comment", "name": "Telegram"},
                        "email": {"icon": "envelope", "name": "Email"},
                        "phone": {"icon": "phone", "name": "Phone"},
                        "manual": {"icon": "edit", "name": "Manual"}
                    }
                    platform_data = platform_info.get(source, {"icon": "comment", "name": source.title()})
                    
                    # Safely access athlete data
                    athlete_id = row[5] if row_length > 5 else None
                    athlete_name = row[10] if row_length > 10 and row[10] else "Unknown Athlete"
                    sport = row[11] if row_length > 11 and row[11] else ""
                    level = row[12] if row_length > 12 and row[12] else ""
                    category = row[6] if row_length > 6 and row[6] else "general"
                    priority = row[7] if row_length > 7 and row[7] else "medium"
                    notes = row[8] if row_length > 8 and row[8] else ""
                    
                    # Create record object
                    record = {
                        "id": row[0] if row_length > 0 else 0,
                        "timestamp": timestamp,
                        "date": date_obj.strftime("%Y-%m-%d") if date_obj else "",
                        "time": date_obj.strftime("%H:%M") if date_obj else "",
                        "transcription": row[2] if row_length > 2 and row[2] else "",
                        "response": row[3] if row_length > 3 and row[3] else "",
                        "source": source,
                        ...
                        "priority": priority,
                        "notes": notes,
                        "title": f"{athlete_name} via {platform_data['name']}" if athlete_name != "Unknown Athlete" else f"Conversation via {platform_data['name']}",
                        "subtitle": (row[2] if row_length > 2 and row[2] else "")[:100] + "..." if len(row[2] if row_length > 2 and row[2] else "") > 100 else (row[2] if row_length > 2 and row[2] else ""),
                        "highlights": 0  # TODO: Add highlights count
                    }
            ...
            if platform != "all":
                ...
                count_cursor = conn.execute(count_query, [platform] if platform != "all" else [])
            else:
        ...
        return JSONResponse(
            {"status": "error", "message": f"Error fetching history data: {str(e)}"},
            status_code=500
        )

# Todos API endpoints
@app.get("/api/athletes/{athlete_id}/todos", response_class=JSONResponse)
async def get_athlete_todos(athlete_id: int) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"status": "error", "message": f"Error fetching todos: {str(e)}"},
            status_code=500
        )

@app.post("/api/athletes/{athlete_id}/todos", response_class=JSONResponse)
async def create_athlete_todo(
    ...
    """Create a new todo for an athlete."""
    try:
        with conn:
            cursor = conn.execute(
                """
                INSERT INTO athlete_todos (athlete_id, title, description, priority, status, due_date, created_at)
                VALUES (?, ?, ?, ?, 'pending', ?, CURRENT_TIMESTAMP)
                """,
                (athlete_id, title, description, priority, due_date)
            )
        ...
        return JSONResponse(
            {"status": "error", "message": f"Error creating todo: {str(e)}"},
            status_code=500
        )

@app.put("/api/athletes/todos/{todo_id}", response_class=JSONResponse)
async def update_athlete_todo(
    todo_id: int,
    title: str = Form(...),
    description: str = Form(""),
    priority: str = Form("medium"),
    ...
    """Update an athlete todo."""
    try:
        with conn:
            cursor = conn.execute(
                """
                UPDATE athlete_todos 
                SET title = ?, description = ?, priority = ?, status = ?, due_date = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                """,
                (title, description, priority, status, due_date, todo_id)
            )
        ...
        return JSONResponse(
            {"status": "error", "message": f"Error updating todo: {str(e)}"},
            status_code=500
        )

@app.delete("/api/athletes/todos/{todo_id}", response_class=JSONResponse)
async def delete_athlete_todo(todo_id: int) -> JSONResponse:
    """Delete an athlete todo."""
    try:
        with conn:
            cursor = conn.execute(
                "DELETE FROM athlete_todos WHERE id = ?",
                (todo_id,)
            )
        ...
        return JSONResponse(
            {"status": "error", "message": f"Error deleting todo: {str(e)}"},
            status_code=500
        )

# Generate highlight from text
@app.post("/api/athletes/{athlete_id}/highlights/generate-from-text", response_class=JSONResponse)
async def generate_highlight_from_text(
    athlete_id: int,
    text: str = Form(...),
    category: str = Form("general"),
    source: str = Form("manual")
) -> JSONResponse:
    """Generate a highlight from selected text using GPT-4o-mini."""
    try:
        # Use GPT-4o-mini to generate a highlight
        prompt = f"""
        Based on this text from an athlete conversation, create a concise highlight that captures the key insight or action item.
        
        Text: "{text}"
        
        Generate a highlight that is:
        1. Concise and actionable
        2. Relevant for athlete coaching
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are an expert sports coach who creates concise, actionable highlights from athlete conversations."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=100,
                temperature=0.7
            )
            highlight_text = response.choices[0].message.content.strip()
        except Exception as e:
            # Fallback to simple text processing
            highlight_text = text[:200] + "..." if len(text) > 200 else text
        ...
        logger.error(f"Error generating highlight from text: {e}")
        return JSONResponse(
            {"status": "error", "message": f"Error generating highlight: {str(e)}"},
            status_code=500
        )
...
@app.get("/api/athletes/{athlete_id}/timeline", response_class=JSONResponse)
async def get_athlete_timeline(athlete_id: int) -> JSONResponse:
    """Get timeline entries for an athlete."""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        cursor.execute("""
            SELECT id, type, platform, direction, date, content, created_at
        ...
        logger.error(f"Error getting athlete timeline: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")


@app.post("/api/athletes/timeline", response_class=JSONResponse)
async def create_timeline_entry(
    ...
    """Create a new timeline entry for an athlete."""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        cursor.execute("""
            INSERT INTO timeline_entries (athlete_id, type, platform, direction, date, content, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (athlete_id, type, platform, direction, date, content, datetime.now().isoformat()))
